      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // points to draw the spline
            vector<double> spline_wp1 = getXY(car_s + 30, (2+4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
            vector<double> spline_wp2 = getXY(car_s + 60, (2+4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
            vector<double> spline_wp3 = getXY(car_s + 90, (2+4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);

            ptsx.push_back(spline_wp1[0]);
            ptsx.push_back(spline_wp2[0]);
            ptsx.push_back(spline_wp3[0]);

            ptsy.push_back(spline_wp1[1]);
            ptsy.push_back(spline_wp2[1]);
            ptsy.push_back(spline_wp3[1]);

            // local coordinates to make math easier
